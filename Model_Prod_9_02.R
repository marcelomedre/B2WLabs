...
###
# 2 models - one up to July and the other starting in August

P9_sales <- P9_by_DATE
P9_sales$DATE_ORDER <- ymd(P9_sales$DATE_ORDER)
P9_sales$month <- as.numeric(as.factor(month(P9_sales$DATE_ORDER)))
P9_sales_jul <- subset(P9_sales, P9_sales$month < 6)
P9_sales_ago <- subset(P9_sales, P9_sales$month > 5)

# Modeling
plot(P9_sales_jul$Price, log(P9_sales_jul$QTY_ORDER)) #days
plot(P9_sales_ago$Price, log(P9_sales_ago$QTY_ORDER)) 


P9_sales_jul$DATE_ORDER <- ymd(P9_sales_jul$DATE_ORDER)
P9_sales_jul$DATE_ORDER <- as.numeric(P9_sales_jul$DATE_ORDER)


regP9_jul <- glm(log(QTY_ORDER) ~ Price + DATE_ORDER, data = P9_sales_jul)
summary(regP9_jul)
fit_regP9_jul <- predict(regP9_jul, newdata = P9_sales_jul, type = "response")
plot(P9_sales_jul$Price, log(P9_sales_jul$QTY_ORDER), 
     main = "P9 Sales X Price - GLM Model",
     xlab = "Price", ylab = "log(QTY ORDER)")
points(P9_sales_jul$Price, fit_regP9_jul, col = "red")
legend("topright", inset=.05, c("black = Obs.","red = Predicted"))

rdois_exp_opt <- lm(log(P9_sales_jul$QTY_ORDER) ~ fit_regP9_jul)
summary(rdois_exp_opt)$r.squared
mean(rdois_exp_opt$residuals)
# r^2 = 0.59.

P9_sales_jul$PROD_ID <- NULL
P9_sales_jul$REVENUE <- NULL
P9_sales_jul <- P9_sales_jul[,c(1, 3, 2)]
P9_sales_jul$QTY_ORDER <- log(P9_sales_jul$QTY_ORDER)

feats <- names(P9_sales_jul)[-(3)]
f <- paste(feats, collapse = " + ")
f <- as.formula(paste("QTY_ORDER ~", f))
f
...
# MLP Model
library(nnet)
maxs <- apply(P9_sales_jul, 2, max)
mins <- apply(P9_sales_jul, 2, min)
P9_scaled <- as.data.frame(scale(P9_sales_jul, center = mins, scale = maxs - mins))
...
results_scaled <- as.data.frame(cbind(results_sc, P9_sales_jul$QTY_ORDER))
names(results_scaled) <- c("Date", "Price", "MLP", "Quantity")
...
ggplot()+
  geom_point(data = results_scaled, aes(x = 1:135, y = Quantity, color = Price,
                                        size = Price))+
  geom_line(data = results_scaled, aes(x = 1:135, y = Quantity), col = "blue")+
  geom_point(data = results_scaled, aes(x = 1:135, y = MLP, size = Price), pch = 4, col = "red")+
  xlab("Days")+
  ylab("Daily Sales Product 9")+
  ggtitle("Weekly Sales Product 9 - MLP Model")+
  theme(axis.text=element_text(size=12, face = "bold"),
        axis.title=element_text(size=14,face="bold"))+
  annotate("text", x = 10, y = 800, label = "R^2 = 0.78")
#******************************************************************************

P9_sales_ago$PROD_ID <- NULL
P9_sales_ago$REVENUE <- NULL
P9_sales_ago <- P9_sales_ago[,c(1, 3, 2)]
P9_sales_ago$QTY_ORDER <- log(P9_sales_ago$QTY_ORDER)

P9_sales_ago$DATE_ORDER <- ymd(P9_sales_ago$DATE_ORDER)
P9_sales_ago$DATE_ORDER <- as.numeric(P9_sales_ago$DATE_ORDER)

feats <- names(P9_sales_ago)[-(3)]
f <- paste(feats, collapse = " + ")
f <- as.formula(paste("QTY_ORDER ~", f))
f
...
# MLP Model
library(nnet)
maxs <- apply(P9_sales_ago, 2, max)
mins <- apply(P9_sales_ago, 2, min)
P9_scaled <- as.data.frame(scale(P9_sales_ago, center = mins, scale = maxs - mins))
...
results_scaled <- as.data.frame(cbind(results_sc, P9_sales_ago$QTY_ORDER))
names(results_scaled) <- c("Date", "Price", "MLP", "Quantity")
...
ggplot()+
  geom_point(data = results_scaled, aes(x = 1:75, y = Quantity, color = Price,
                                        size = Price))+
  geom_line(data = results_scaled, aes(x = 1:75, y = Quantity), col = "blue")+
  geom_point(data = results_scaled, aes(x = 1:75, y = MLP, size = Price), pch = 4, col = "red")+
  xlab("Days")+
  ylab("Daily Sales Product 9")+
  ggtitle("Weekly Sales Product 9 - MLP Model")+
  theme(axis.text=element_text(size=12, face = "bold"),
        axis.title=element_text(size=14,face="bold"))+
  annotate("text", x = 10, y = 800, label = "R^2 = 0.87")
